...
#        for i in range(0,n-1):
        for ii in range(0,nr-1):
            ...
            eps = (x[ind[i+1],j]-x[ind[i],j])/2.
            
            splitval = x[ind[i],j]+eps
            # new regions
            ...
            loss[ii,j] = np.square(nleft/float(n))/volume(regionL) + \
                            np.square(nright/float(n))/volume(regionR)
        # end for i    
    #end for j
    maxval = loss.max()
    splitval, splitdim  = np.where(loss==maxval)       
#    print maxval
...
#    loss = np.zeros((2*n,d), dtype = np.float32 )
    nr = n/2;
    loss = np.zeros((2*nr,d), dtype = np.float32 )
    for j in range(0,d):
        print j
        splitdim = j
        ind = np.argsort(node.points[:,j])
...
#    if depth>5:
        ...
        if splitmethod == 'naive':
            ...
            splitval, splitdim = splitclever(parentnode, 0.5)
        # end if naive
        else :
            # select theoretically best split
            splitval, splitdim = splitclever(parentnode, 0.5)            
...
def DT_learning(trainingx, trainingy, c, splitmethod):
    ...
    while stack:
        ...
        # if termination condition is satisfied:    
#        pointsdensity = currentnode.points.shape[0]/float(n)
        # if min density is reached or region has only few points 
        if currentnode.points.shape[0] < 50 :#np.power(n, 1/3.) :
#        if currentnode.p>=0.0001 or pointsdensity<0.001: # if maximal depth of the tree is reached
            leaveslist.append(currentnode)
        else: # if split further
            if splitmethod == 'naive':
                # split value : split on the middle of the interval
                splitval, splitdim = splitnaive(currentnode) #splitnaive(currentnode, 0)#depth)
            # end if naive
            else :
                # select theoretically best split
...
#    if depth>5:
        ...
        splitval, splitdim = splitnaive(parentnode, 0.5)
