...
% calculate number of points with similar descriptors
function [sim] = compair_subgraphs(G1, G2)
    ...
    for i=1:n1
        ...
        sub_G1_V = G1.V(ind_V_subG1, 1:2);
        
        A1cut = A1(ind_V_subG1, ind_V_subG1');    
        dist1 = squareform(pdist(sub_G1_V, 'euclidean'));
        dist1(~A1cut) = NaN;
    ...
    for j=1:n2
        ...
        sub_G2_V = G2.V(ind_V_subG2, 1:2);

        A2cut = A2(ind_V_subG2, ind_V_subG2');    
        dist2 = squareform(pdist(sub_G2_V, 'euclidean'));
        dist2(~A2cut) = NaN;
