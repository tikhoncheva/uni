...
OptScore = zeros(length(settings{Con}{4}), 1, Set.nTest);
...
for kk = 1:Set.nTest, fprintf('Test: %d of %d ', kk, Set.nTest);
    for i = 1:length(settings{Con}{4})
        ...
        OptScore(i,1,kk)  = problem.GToptscore;
...
%% optimal score
% Xopt = problem.GTbool; Xopt = reshape(Xopt, problem.nP1, problem.nP2);
% [corr(:,1), corr(:,2)] = find(Xopt); optscore = matching_score_LL(problem.LLG1, problem.LLG2, corr);