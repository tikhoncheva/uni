...
% candidate matches as used in ProgGM
candMatches = cell(size(LLG1.V,1),1);
for i = 1:size(LLG1.V,1)
    candmatches_i =  cand_matchlist(:,1) == i;
    candMatches{i} = cand_matchlist(candmatches_i,2);
end
% all possible correspondences between two sets
% candMatches = {(1:size(LLG2.V,1))};
% candMatches = repmat(candMatches, size(LLG1.V,1), 1);
