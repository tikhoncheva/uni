function [accuracy, score, score2, time, X, perform_data] = run_algorithm(alg, invar)
...
if strcmp(alg, 'wrapper_TwoLevelGM')
    start = tic;
    [score2, X, perform_data] = wrapper_TwoLevelGM(invar.LLG1, invar.LLG2, invar.cdata.GT);
    time = toc(start);
end  
...
if strcmp(alg, 'wrapper_featureMatching')
    start = tic;
    [score2, X, perform_data] = wrapper_featureMatching(invar.LLG1, invar.LLG2, invar.cdata.GT);
    time = toc(start);
end      
