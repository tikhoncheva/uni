function [ cand_matchlist ] = selectCandidateMatch( voting_space, feat1, desc1, feat2, desc2, max_candidates, threshold_dissim, mparam )
...
mparam.bFilterMatch = 1; % filtering matches
...
if mparam.bFilterMatch
    ...
    for  iter_cand = nSelMatch+1:nMax
        cur_iv = iv(iter_cand);
        ...
        if ~bSelf
            ...
            %idx = rangesearch(XY_sel_view1,XY_view1,mparam.redundancy_thres);
            %ridx_view1 = idx{1};
            %idx = rangesearch(XY_sel_view2,XY_view2,mparam.redundancy_thres);
            %ridx_view2 = idx{1};
        ...
        if isempty(equi_ridx) % if there's no equi matches
            desc_sqdist = sum((desc2(ind_feat1(cur_iv))-desc2(ind_feat2(cur_iv))).^2);
