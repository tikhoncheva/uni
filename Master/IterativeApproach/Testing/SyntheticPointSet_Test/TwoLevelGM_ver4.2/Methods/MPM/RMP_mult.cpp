                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    G_row = mxGetIr(prhs[2]);	// each element in the ir array indicates a row (offset by 1) at which a nonzero element can be found
    G_col = mxGetJc(prhs[2]); // The jc array is an index that indirectly specifies a column where nonzero elements can be found
    ...
    for (j = 0; j < mrows; j++)	// j=ia
    { 
        for (g = 0; g < ngroups; ++g)	// j \in N_i
        {
            vmax = -9999;
            bSame = false;
            
            for (ge = G_col[g]; ge < G_col[g+1]; ++ge) 	// b \in N_a
            {
		// G_row[ge] = jb
                if ( G_row[ge] == j )
                {
