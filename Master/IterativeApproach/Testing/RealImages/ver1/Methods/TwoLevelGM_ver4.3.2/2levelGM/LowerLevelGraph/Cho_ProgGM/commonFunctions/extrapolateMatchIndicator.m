...
function [ indicator_ext ] = extrapolateMatchIndicator(V1, V2, matchList, indicator, search_radius )
...
if isempty(indicator)
    ...
    kdtreeNS1 = kdtree_build(V1(matchList(:,1),1:2));%KDTreeSearcher( view(view1).feat(matchList(:,1),1:2) );
    kdtreeNS2 = kdtree_build(V2(matchList(:,2),1:2));%KDTreeSearcher( view(view2).feat(matchList(:,2),1:2) ); 
    curMatchIdx = find(indicator);
    nCurMatch = length(curMatchIdx);
    indicator_ext = zeros(1,size(matchList,1));
    
    for i=1:nCurMatch
        query_view1 = V1(matchList(curMatchIdx(i),1),1:2);
        query_view2 = V2(matchList(curMatchIdx(i),2),1:2);
        if ~bSelf
