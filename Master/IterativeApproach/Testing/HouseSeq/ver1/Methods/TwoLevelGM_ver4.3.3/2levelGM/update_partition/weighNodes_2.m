...
%             bi = [H(1,3); H(2,3)];
            ...
            ind = [];
            ind_new = (1:size(Vai_m,1))';
            
            while ~isequal(ind, ind_new) && numel(ind_new)>1
                
                ind = ind_new;
                [Transform, ~]=cpd_register(Vaj_m(ind,1:2), Vai_m(ind,1:2), opt); 
                Ai = Transform.R;
                bi = Transform.t;
                ...
                err_vect1 = sqrt((Vaj_m(ind,1)-PVai_m(ind,1)).^2+(Vaj_m(ind,2)-PVai_m(ind,2)).^2);              
                
                ind_new = ind(err_vect1<0.01);
                
            end
            
            % apply transformation to the whole set of nodes
            PVai_m = Ai * Vai_m' + repmat(bi,1,size(Vai_m,1)); % proejction of Vai_m nodes
            PVai_m = PVai_m';
...
%             bj = [ inverseH(1,3); inverseH(2,3)]; 
            ...
            ind = [];
            ind_new = (1:size(Vaj_m,1))';
            
            while ~isequal(ind, ind_new) && numel(ind_new)>1         
                ind = ind_new;
                [Transform, ~]=cpd_register(Vai_m(ind,1:2), Vaj_m(ind,1:2), opt);
                Aj = Transform.R;
                bj = Transform.t;
                ...
                err_vect2 = sqrt((Vai_m(ind,1)-PVaj_m(ind,1)).^2+(Vai_m(ind,2)-PVaj_m(ind,2)).^2);                         
                ind_new = ind(err_vect2<0.01);
            end
