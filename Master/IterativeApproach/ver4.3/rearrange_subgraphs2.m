...
%    tic
   ...
   new_HLG1_U = 0.5*double(old_HLG1_U); 
   new_HLG2_U = 0.5*double(old_HLG2_U); 
...
%             err = 0.5*(err1 + err2);
            ...
            if (err<error_eps)  % Rule 1
                ...
                new_HLG1_U(nn_PVaj, ai) = exp(-err);
                
                new_HLG2_U(ind_Vaj, aj) = exp(-err);
                new_HLG2_U(nn_PVai, aj) = exp(-err);
