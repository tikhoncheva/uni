...
affmatrix_HLG = HLGmatches_it.affmatrix;               % affinity matrix between graphs on the higher level
                                                       % size (nVA1*nVA2)x(nVA1*nVA2)
...
for k = 1:npairs % for each match ai<->aj on the High Level
    ...
    ai = matched_pairs_HLG(k,1);    % ai<->aj,
    ...
    ind_adj_edges = find( affmatrix_HLG(ind_ai_aj,:) ); % row 
    ...
    for i=1:numel(ind_adj_edges)    % ai<->aj, ak<->al
        ...
        vj = find(LLG2.U(:,aj)>0);        % nodes that belongs to the anchor aj
        nV2 = numel(vj);
        vj_al = find(LLG2.U(vj, al));     % nodes that also belongs to the anchor al
