...
function [U, affected_anchors] = randomly_shift_nodes(LLG, HLG)
    ...
    ind_picked_nodes = datasample(1:nV, m,'Replace',false)';

    dist_to_anchors = repmat(LLG.V(ind_picked_nodes,1:2), nA,1) ...
    ...
    dist_to_anchors(U(ind_picked_nodes, :)) = Inf;
    ...
    [~, affected_anchors] = find(U(ind_picked_nodes, :));
    affected_anchors = unique(affected_anchors);
    affected_anchors = [affected_anchors; nn_anchors_ind];

    U(ind_picked_nodes, :) = 0;
    ind = sub2ind(size(U), ind_picked_nodes, nn_anchors_ind);
    U(ind) = 1;
