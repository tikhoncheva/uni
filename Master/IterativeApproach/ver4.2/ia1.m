...
%                                       handles.parameters.nSP1);  
    LLG1 = handles.LLG1;
%     LLG1.W = ones(size(LLG1.V,1),1)*Inf;
%     LLG1.W = ones(size(LLG1.V,1),1)*NaN;
...
%         y_ac(i) = TP/ size(handles.LLGmatches(i).matched_pairs,1) * 100;
        y_ac(i) = TP/ size(GT,1) * 100;
        if i>1
            ind = ismember(handles.LLGmatches(i).matched_pairs(:,1:2), ...
                   handles.LLGmatches(i-1).matched_pairs(:,1:2));
            numel(find(ind==0))
        end
    end
