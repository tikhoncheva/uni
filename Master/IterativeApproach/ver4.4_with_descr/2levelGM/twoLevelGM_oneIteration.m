...
function [HLG1, HLG2, LLGmatches, HLGmatches, affTrafo] = ...
    ...
    if it>1
        ...
        % -----------------------------------------------------------------------     
        [LLG1, LLG2, HLG1, HLG2, affTrafo] = MetropolisAlg(it-1, LLG1, LLG2, HLG1, HLG2,...
...
%         err = zeros(np, it-1);
%         for i = 1:it-1
%             ind_pairs = HLGmatches(it-1).matched_pairs(:,1:2);
%             ind = (ind_pairs(:,2)-1)*size(HLG1.V,1)+ind_pairs(:,1);
%             err(ind, i) = affTrafo{it-1}(:,3);
%         end
%         
%         err = min(err,[],2);
%         pen_vec = exp(-err);
%             
%         affmatrix(1:np+1:end) = affmatrix(1:np+1:end) * pen_vec;
