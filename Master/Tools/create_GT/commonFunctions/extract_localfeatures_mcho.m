function [ featInfo ] = extract_localfeatures_mcho( filePathName, fparam, bShowFeat, F )
...
for i=1:length(featExt)
    if fparam.bFeatExtUse(i)
        if i==length(featExt)
            tmpFeat = [F, repmat([1,0,1], size(F,1),1)];
        ...
        featInfo.feat = [ featInfo.feat; tmpFeat ];
        featInfo.nFeatOfExt(i) = nTmpFeat;
