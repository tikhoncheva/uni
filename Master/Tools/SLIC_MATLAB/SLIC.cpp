...
void SLIC::DoRGBtoLABConversion(
	...
	double**&					bvec)
{
	lvec = new double*[m_depth];
	avec = new double*[m_depth];
	bvec = new double*[m_depth];

	int sz = m_width*m_height;
	for( int d = 0; d < m_depth; d++ )
	{
		lvec[d] = new double[sz];
		avec[d] = new double[sz];
		bvec[d] = new double[sz];
	...
	const string&				path) 
{
	...
	const char *pch = filename.c_str();
	int i;
	for (i = 0; pch[i] != '\0' && pch[i] != '.'; i++) 
		fname[i] = pch[i];
	fname[i] = '\0';
