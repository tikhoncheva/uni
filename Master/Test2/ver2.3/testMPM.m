...
%     conflictMatrix = getConflictMatrix(group1, group2);
    ...
    nAffMatrix = nnz(E12); % size(matchInfo.match, 2);
...
%     edge similarity (non-diagonal elements of the affinity matrix)
    ...
    for ia = 1:size(cand_matchlist_uniq,2) %1:nAffMatrix
        i = cand_matchlist_uniq(1, ia);
        a = cand_matchlist_uniq(2, ia);
        
        for jb = 1:size(cand_matchlist_uniq,2) %1:nAffMatrix
...
%     for j = 1:size(cand_matchlist_uniq,2)
%         ia = cand_matchlist_uniq(:,j);
%         [~, ind] = ismember([ia(1), ia(2) ], L12, 'rows');
%         D(ind, ind) = matchInfo.sim(j);
%     end
    
    meanD = sum(D(:))/size(find(D(:)>0),1);
%     AffMatrix = exp(-D.^2/meanD^2);
...
%         ia = cand_matchlist_uniq(:,j);
%         [~, ind] = ismember([ia(1), ia(2) ], L12, 'rows');
%         AffMatrix(ind, ind) = matchInfo.sim(j);
%     end
