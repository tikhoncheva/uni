...
It is pleasant to see that 2LevelGM is able to find the absolutely correct matching for the whole set except one image pairs and therefore shows better results in objective score and accuracy than ProgGM does. The result of matching of the last image pair is \ToDo{roughly} the same for both algorithms. We notice, that the 2LevelGM is not able to improve graph partitioning further in this case, although some nodes from the true matches belonged to different subgraphs. Regarding the running time ProgGM is faster. However we \ToDo{should point out} may notice, that ProgGM \ToDo{directly} stops as soon as the matching score does not increase any more. In contrast to that, our 2LevelGM has to make some additional iterations at the end to be sure of finding local minimum. This is done intentionally, as we cannot make \ToDo{certain} any statement about convergence of our framework.
