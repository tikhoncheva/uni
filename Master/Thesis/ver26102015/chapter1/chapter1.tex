...
As we mentioned above exact graph matching problems are often not applicable to real-world problems. There are mainly two reasons for that. %\ToDo{On the one hand, there are natural variations in the structure of graphs, that describe the same object}
On the one hand, the same object can be described by graphs with different structures. %Those variations
...
We start first with an even stricter assumption, that $n_1=n_2=n$ and that the matching is bijective. In this case a mapping between nodes of two graphs defines a permutation $\sigma$ of the set $\{1,\dots,n\}$. Each permutation $\sigma$ can be represented by the permutation matrix $P=\{P_{ij}\}$, where
\begin{equation*}
P_{ij}=\begin{cases}
 1, & \text{if } \sigma(i)=j, \\
 0, & \text{otherwise.} \\
...
Consider two attributed graphs $G^I = (V^I, E^I,D^I)$ and $G^J = (V^J, E^J,D^J)$ and a matching $m$ between subsets $\bar{V}^I,\bar{V}^J$ of $V^I$ and $V^J$ respectively. The edit operations on nodes can be directly defined by the mapping $m$~\cite{Bunke1998_ErrTolerantGM}:
\begin{itemize}
