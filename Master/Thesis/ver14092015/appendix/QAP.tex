...
\end{align}
Based on this formulation the matrix $B$ is called sometimes \emph{linear cost matrix} and the matrix $D\otimes F$ \emph{quadratic costs matrix}. If we assume the matrix $B$ be a constant matrix with the same values, that it does not have any influence on the optimization process. In this case setting matrix $F$ to $-(A^I)^T$ and matrix $D$ to $(A^J)^T$ lead us to the formulation~\eqref{eq:QAP2}. Similarly, denoting the Kronecker product $(-D\otimes F)$ as $S$ results in the formulation~\eqref{eq:sumQAP}.

It remains to show, that \eqref{eq:QAP1} is equivalent to \eqref{eq:QAP_classic3}. Indeed, consider the first term of the objective function in~\eqref{eq:QAP1}:
\begin{align*}
...
\end{align*}
The first and the last term are obviously constant, and thus can be ignored in optimization. Finally, substitution of $F=-(A^I)^T$, $D=(A^J)^T$ leads us to the formulation $\tr(F\hat{P}D^T\hat{P}^T)$. After performing the similar calculations on the second term of the objective in~\eqref{eq:QAP1} we obtain the problem~\eqref{eq:QAP_classic3}. That is exactly, what we wanted to show.
