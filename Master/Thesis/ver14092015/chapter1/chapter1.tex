...
The objective function of \eqref{eq:QAP2} is a negation of the objective of the \emph{quadratic assignment problem}% with linear cost matrix set to zero
...
Similar to the exact graph matching problems the tree based methods with backtracking were successfully applied also to inexact matching problems. To determine the tree traverse direction and/or to bound unpromising branches one uses score of the current achieved partial matching together with a heuristic estimate of the score for the remaining nodes. Those methods % using $A^*-$Algorithms~\ToDo{cite} for tree searching 
were especially wild used for the error-correcting graph matching.
The first such algorithm, which guaranties to find the optimal solution, was presented in~\ToDo{cite}. For further examples of the three search methods we refer to \cite{Shapiro1981,Wang1995}.
