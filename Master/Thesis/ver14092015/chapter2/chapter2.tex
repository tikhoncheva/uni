...
\end{alignat}
A vector $x$ is an indicator vector of a subset of a set of all possible correspondences between the nodes of two graphs. That means, that element $x_k$ of this vector is equal $1$ if and only if the corresponding node pair $(v_i,v_{j}),v_i\in V^I,v_{j}\in V^J$, is selected into subset. The constraints~\eqref{eq:gQAP3_2},~\eqref{eq:gQAP4_2} ensure, that each node of the graph $G^I$ can be matched to exactly one node of the second graph $G^J$.
The information about similarity of two graphs is enclosed into matrix $S\in\mathbb{R}^{n_1n_2\times n_1n_2}$. Rows (columns) of this matrix correspond to all possible matches of nodes in $V^I$ and $V^J$.  The diagonal elements of this matrix contain similarities between matched nodes. On the other side, the non-diagonal elements of $S$ measure an agreement between two pairs of matched nodes to be present in an optimal matching. Our aim is to find a subset of $n_1n_2$ possible correspondences between the nodes of two graphs, which maximizes the similarity value between those graphs. 
